...
#: ``<workspace>/AcouDiet/ai/src/config.py``
AI_ROOT = Path(__file__).resolve().parents[1]        # <workspace>/AcouDiet/ai
PROJECT_ROOT = AI_ROOT.parent                        # <workspace>/AcouDiet
#: The SSOT lives at the workspace root in this repository layout
#: (``<workspace>/shared/feature_config.json``), i.e. one level above ``AcouDiet/``.
