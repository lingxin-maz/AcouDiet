...
> 不同，本轮交付的 `models/v1.3/models/acoudiet_fp32.tflite` sha256 是 `31fba3ec…19f4`，
...
```powershell
python tool\check_apk_contents.py <你的.apk> --expect-no-internet
#   → 包内 .tflite 的 sha256 / 字节数与**包内** model_card.json 逐项一致？
#     （ADR-32 起由卡片驱动；此前写死 v1.1 的哈希，换模型后会对合法包打印 False）
#     该检查器有两个方向：--expect-no-internet（offline 风味）/ --expect-internet（agent 风味）
...
pwsh -File tool\verify_all.ps1 # 全套回归（22 步）
```
