        ...
        children: [
          Expanded(
            child: ConstrainedBox(
              ...
              //
              // ⚠️ REPORTED DEFECT ("按钮无作用"). These two were always enabled, and their
              // handlers were `void` methods that used `_handle?.` -- so with no live session a tap
              // did NOTHING **and still set the state to `listening`**. The card could stay on
              // screen after a session ended (the prediction view is not cleared on stop), which
              // left two buttons that looked tappable, did nothing, and made the page claim it was
              // listening. Disabled is the honest rendering: there is no session to answer.
              child: FilledButton(
                onPressed: notifier.sessionRunning
                    ? () => notifier.answerConfirmation(accepted: true)
                    : null,
          ...
          Expanded(
            child: ConstrainedBox(
              ...
              child: OutlinedButton(
                ...
                onPressed:
                    notifier.sessionRunning ? () => notifier.rejectSuggestion() : null,
