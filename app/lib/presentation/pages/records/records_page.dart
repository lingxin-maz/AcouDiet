...
class DayGroupHeader extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // ONE date, in ONE style.
    //
    // Two defects were reported against the previous version, and the code shows both:
    //
    //   * for any day older than yesterday, `dayGroupHeader` returns `M月d日` and `dayDateLabel`
    //     returns the SAME `M月d日` -- so the date was printed twice, side by side;
    //   * when the two did differ (`今天` + `9月10日`) they were rendered at DIFFERENT sizes
    //     (`metric.copyWith(fontSize: 16)` vs `bodyMuted`), which reads as a rendering bug rather
    //     than as two pieces of information.
    //
    // Both halves genuinely carry something: the relative word is what a user scans for, and
    // ADR-24 added the calendar date because the relative word alone loses the date once the
    // list is scrolled. So the fix keeps BOTH and makes them one typographic unit -- same style,
    // one separator -- instead of deleting information to make the line look tidy.
    final label = AcouFormat.dayHeaderLabel(header, dateLabel);
    return Padding(
      ...
      child: Row(
        children: [
          Text(label, style: AcouTheme.metric.copyWith(fontSize: 16)),
          const SizedBox(width: AcouTheme.spaceXs),
