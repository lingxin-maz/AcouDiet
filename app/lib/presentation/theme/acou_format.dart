...
abstract final class AcouFormat {
  ...
  /// The day-group header as ONE string (`ADR-45`), given the two halves the day-group logic
  /// produces.
  ///
  /// **Why this exists as a rule rather than as two `Text` widgets in the page.** The previous
  /// rendering put [dayGroupHeader] and [dayDateLabel] side by side at DIFFERENT styles, and both
  /// defects that produced are visible in the data alone:
  ///
  ///   * for any day older than yesterday, `dayGroupHeader` returns `M月d日` and `dayDateLabel`
  ///     returns the SAME `M月d日` -- the date printed twice;
  ///   * when they differ (`今天` + `9月10日`), two styles made the line look like a rendering bug
  ///     rather than like two pieces of information.
  ///
  /// Both halves still carry something, so neither is dropped: the relative word is what a user
  /// scans for, and ADR-24 added the calendar date because the relative word alone loses the date
  /// once the list is scrolled. They are joined into one typographic unit instead.
  ///
  /// It lives here, next to the other day-formatting rules, so it is a PURE function that the
  /// offline presenter suite can assert -- the bug it fixes was in a widget, where nothing could.
  static String dayHeaderLabel(String header, String dateLabel) {
    if (dateLabel.isEmpty || dateLabel == header) return header;
