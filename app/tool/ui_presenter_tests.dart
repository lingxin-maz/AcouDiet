...
void _formatChecks() {
  ...
  // ---- ADR-45: the day header is ONE date, in ONE style ---------------------------------------
  //
  // Reported defect: "日期重复体现大小不一字体". The old rendering printed `dayGroupHeader` and
  // `dayDateLabel` as two `Text`s at two different styles, and the duplication is visible in the
  // data itself: for any day older than yesterday the two functions return the SAME string.
  final older = AcouFormat.dayGroupHeader(
      dayKey: '2026-09-03', todayKey: '2026-09-10', yesterdayKey: '2026-09-09');
  final olderDate = AcouFormat.dayDateLabel('2026-09-03');
  eq('an older day really does produce the same string twice (the duplication is in the data)',
      older, olderDate);
  eq('so the header collapses it to one', AcouFormat.dayHeaderLabel(older, olderDate), '9月3日');
  eq('and it appears exactly once', '9月3日'.allMatches(
          AcouFormat.dayHeaderLabel(older, olderDate)).length, 1);

  final today = AcouFormat.dayGroupHeader(
      dayKey: '2026-09-10', todayKey: '2026-09-10', yesterdayKey: '2026-09-09');
  final todayDate = AcouFormat.dayDateLabel('2026-09-10');
  eq('today keeps BOTH halves, joined (ADR-24 added the calendar date for a reason)',
      AcouFormat.dayHeaderLabel(today, todayDate), '今天 · 9月10日');
  eq('an empty second half leaves the header alone',
      AcouFormat.dayHeaderLabel('今天', ''), '今天');

  // Negative control: the OLD two-widget rendering is exactly what must not come back. If the
  // collapse is removed, the older-day case produces the date twice -- which is what this asserts.
  check('negative control: the un-collapsed header really does duplicate the date',
      '$older$olderDate' == '9月3日9月3日' &&
          AcouFormat.dayHeaderLabel(older, olderDate) != '$older$olderDate');
...
void _settingsChecks() {
  ...
  // ADR-44 (`SPEC-U-07` section 4.3): the "no network permission" sentence is a claim about the
  // OFFLINE package's manifest and is therefore FLAVOUR-parameterised. It must be present in the
  // offline composition and absent from the agent one, while both keep the two sentences that are
  // true of every build.
  final offlineNotice = '${UiStrings.privacyNoticeBase}${UiStrings.privacyNoticeOfflineSuffix}';
  check(
      'the privacy notice claims no network permission only in the offline flavour',
      offlineNotice.contains('不申请网络权限') &&
          !UiStrings.privacyNoticeBase.contains('不申请网络权限') &&
          offlineNotice.contains('音频永不离开设备') &&
          UiStrings.privacyNoticeBase.contains('音频永不离开设备') &&
          offlineNotice.contains('云端智能默认关闭') &&
          UiStrings.privacyNoticeBase.contains('云端智能默认关闭'),
      'offline=$offlineNotice agent=${UiStrings.privacyNoticeBase}');
  eq('the live privacy notice follows the compile-time flavour', view.privacyNotice,
      isOfflineFlavour ? offlineNotice : UiStrings.privacyNoticeBase);
  check('the privacy notice says audio is not written to storage',
      view.privacyNotice.contains('不写入存储'));
  check('the dialog copy makes the same flavour-dependent claim',
      isOfflineFlavour
          ? UiStrings.privacyDialogBody.contains('不申请网络权限')
          : !UiStrings.privacyDialogBody.contains('不申请网络权限'));
...
void _agentChecks() {
  group('U-07 agent (ADR-44)');

  // The state machine is frozen at eight states and the failure sub-reasons at nine
  // (SPEC-U-07 section 2.3): the reasons do NOT enter the machine, they only pick the sentence.
  eq('the agent page has exactly eight UI states', AgentUiState.values.length, 8);
  check(
      'the eight states are the frozen set',
      AgentUiState.values.map((e) => e.name).join(',') ==
          'unconsented,keyMissing,offline,idle,sending,streaming,toolRunning,failed',
      AgentUiState.values.map((e) => e.name).join(','));
  eq('the failure sub-reasons are the frozen nine', AgentFailureReason.values.length, 9);

  // Code -> reason -> sentence, one row at a time.
  eq('offline maps to the network reason', AgentPresenter.failureOf(Errors.agent(Codes.agentOffline)),
      AgentFailureReason.network);
  eq('401 maps to an invalid key',
      AgentPresenter.failureOf(Errors.agent(Codes.agentUnauthorized)),
      AgentFailureReason.invalidKey);
  eq('429 maps to the rate-limit reason',
      AgentPresenter.failureOf(Errors.agent(Codes.agentRateLimited)),
      AgentFailureReason.rateLimited);
  eq('5xx maps to the server reason',
      AgentPresenter.failureOf(Errors.agent(Codes.agentServerError)),
      AgentFailureReason.serverError);
  eq('a timeout keeps its own reason',
      AgentPresenter.failureOf(Errors.agent(Codes.agentTimeout)), AgentFailureReason.timeout);
  eq('a broken stream keeps its own reason',
      AgentPresenter.failureOf(Errors.agent(Codes.agentStreamBroken)),
      AgentFailureReason.streamBroken);
  eq('an unparsable tool call is reported as such',
      AgentPresenter.failureOf(Errors.agent(Codes.agentToolCallUnparsable)),
      AgentFailureReason.toolUnparsable);
  eq('an exhausted tool budget is reported as such',
      AgentPresenter.failureOf(Errors.agent(Codes.agentToolRoundsExceeded)),
      AgentFailureReason.toolRoundsExceeded);
  eq('a code outside the agent path maps to nothing', AgentPresenter.failureOf(Errors.kb('x')), null);

  eq('the network sentence is the frozen one',
      AgentPresenter.failureText(AgentFailureReason.network),
      '当前离线，核心功能可正常使用。恢复网络后可继续对话。');
  eq('the key sentence is the frozen one',
      AgentPresenter.failureText(AgentFailureReason.invalidKey),
      'Key 无效或额度不足。请在设置中检查你自己的 API Key。');
  eq('the truncated sentence is the frozen one',
      AgentPresenter.failureText(AgentFailureReason.truncated), '回复不完整（已达输出上限）。');
  eq('the broken-stream sentence is the frozen one',
      AgentPresenter.failureText(AgentFailureReason.streamBroken), '回复不完整（连接已断开）。');
  eq('the tool-budget sentence is the frozen one',
      AgentPresenter.failureText(AgentFailureReason.toolRoundsExceeded),
      '本轮工具调用已达上限，已停止继续尝试。');
  check(
      'rate-limit / server / timeout share one honest sentence',
      AgentPresenter.failureText(AgentFailureReason.rateLimited) == '服务暂时不可用，请稍后再试。' &&
          AgentPresenter.failureText(AgentFailureReason.serverError) == '服务暂时不可用，请稍后再试。' &&
          AgentPresenter.failureText(AgentFailureReason.timeout) == '服务暂时不可用，请稍后再试。');

  check(
      'a truncated or broken answer KEEPS what was received (ACD-AGENT-008)',
      AgentPresenter.keepsPartialText(AgentFailureReason.truncated) &&
          AgentPresenter.keepsPartialText(AgentFailureReason.streamBroken) &&
          !AgentPresenter.keepsPartialText(AgentFailureReason.invalidKey));
  check(
      'only a network failure degrades to the offline state',
      AgentPresenter.degradesToOffline(AgentFailureReason.network) &&
          !AgentPresenter.degradesToOffline(AgentFailureReason.serverError));

  // The key is only ever shown masked (FF-26b).
  eq('a short credential is masked entirely', AgentPresenter.maskedKey('sk-1'), '****');
  check('a real credential keeps only 3 leading and 4 trailing characters',
      AgentPresenter.maskedKey('sk-0123456789abcdef') == 'sk-…cdef',
      AgentPresenter.maskedKey('sk-0123456789abcdef'));
  eq('no stored key reads as the empty marker', AgentPresenter.maskedKey(null), UiStrings.apiKeyNone);

  // The four tools -> four card kinds, with the tool's own summary NEVER rendered.
  final healthCard = AgentPresenter.cardsFrom(
    toolName: AgentToolNames.healthSummary,
    observation:
        const AgentObservation.ok('已读取 week 的本地健康数据', artifacts: <String, Object?>{
      'range': 'week',
      'record_count': 3,
      'snack_count': 1,
      'total_score': 88,
  ...
  eq('the health summary becomes the health card', healthCard.kind, AgentCardKind.health);
  check('the health card carries the range and the counts',
      healthCard.lines.first.contains('本周') && healthCard.lines.first.contains('记录 3 次'),
      healthCard.lines.join(' | '));
  check('the health card carries the total and the grade',
      healthCard.lines.any((l) => l.contains('总分 88 分（良好）')), healthCard.lines.join(' | '));
  check('the tool summary text NEVER renders',
      !healthCard.lines.any((l) => l.contains('已读取')), healthCard.lines.join(' | '));

  final mealsCard = AgentPresenter.cardsFrom(
    toolName: AgentToolNames.recentMeals,
    observation: const AgentObservation.ok('已读取最近 1 条记录', artifacts: <String, Object?>{
      'records': <Object?>[
        <String, Object?>{'class_label': '面条', 'eaten_at_ms': 0, 'confidence': 0.88},
      ],
    }),
    platforms: const <TakeoutPlatform>[],
  ).single;
  eq('the recent meals become the meals card', mealsCard.kind, AgentCardKind.meals);
  check(
      'the meals card slots the class, the time and the confidence',
      mealsCard.lines.single.contains('面条') &&
          mealsCard.lines.single.contains('置信度 88%') &&
          mealsCard.lines.single.contains(':'),
      mealsCard.lines.single);

  final adviceCard = AgentPresenter.cardsFrom(
    toolName: AgentToolNames.recommendFood,
    observation: const AgentObservation.ok('已读取 1 条本地建议', artifacts: <String, Object?>{
      'advice': <Object?>['提供日常健康管理建议，不进行疾病诊断，不替代专业医疗意见'],
    }),
    platforms: const <TakeoutPlatform>[],
  ).single;
  eq('the recommendation becomes the advice card', adviceCard.kind, AgentCardKind.advice);
  eq('the advice lines are the engine\'s own text, verbatim', adviceCard.lines.single,
      AdviceEngine.disclaimerText);
  ...
  final handoff = AgentPresenter.cardsFrom(
    toolName: AgentToolNames.proposeTakeout,
    // NOT `const`, and the url is DERIVED rather than hand-written (ADR-45).
    //
    // The first version of this fixture spelled the card by hand and simply omitted `url`. That
    // made the fixture disagree with what `ProposeTakeoutSearchTool` actually publishes -- and the
    // assertion added for the in-app browser is what surfaced it. A hand-written fixture drifts
    // silently in exactly this way: it keeps passing as long as nothing asserts the field that
    // was left out. Deriving the url from the same function the tool uses removes the class.
    observation: AgentObservation.ok('已生成 1 张交接卡片（未打开任何 App）',
        artifacts: <String, Object?>{
          'cards': <Object?>[
            <String, Object?>{
              ...
              'label': '美团',
              'keyword': '牛肉面',
              'url': takeoutUrlFor(meituan, '牛肉面'),
              'scheme': takeoutSchemeFor(meituan, '牛肉面'),
              'verified': true,
            },
          ],
          'skipped': <Object?>[
    ...
    platforms: const <TakeoutPlatform>[meituan],
  );
  eq('a disabled platform is reported as a card, not silently dropped',
      handoff.length, 2);
  final enabledHandoff = handoff.firstWhere((c) => c.handoff!.enabled).handoff!;
  eq('the handoff title names the platform and the keyword', enabledHandoff.title,
      '帮你想好了，去 美团 搜「牛肉面」');
  eq('the handoff button names the platform', enabledHandoff.buttonLabel, '打开 美团 搜索');
  check('an enabled + verified platform carries no caveat', enabledHandoff.caveats.isEmpty);
  final closedHandoff = handoff.firstWhere((c) => !c.handoff!.enabled).handoff!;
  eq('a disabled platform says so in its own copy', closedHandoff.caveats,
      <String>[UiStrings.agentHandoffClosed]);

  final unverified = AgentHandoffView(
    platformId: 'meituan',
    label: '美团',
    keyword: '牛肉面',
    enabled: true,
  ...
  eq('an unverified platform says so next to its button', unverified.caveats,
      <String>[UiStrings.agentHandoffUnverified]);

  final failedCard = AgentPresenter.cardsFrom(
    toolName: AgentToolNames.healthSummary,
    observation: const AgentObservation.failed(
      summary: '工具执行失败',
      rootCauseHint: 'x',
      safeRetry: 'y',
      stopCondition: 'z',
  ...
  eq('a failed tool becomes the failure card, never its raw summary', failedCard.kind,
      AgentCardKind.toolFailed);
  check('the failure card does not echo the tool text',
      failedCard.lines.isEmpty && failedCard.title == UiStrings.agentCardFailedTitle);

  check('the handoff semantics label carries the platform name',
      enabledHandoff.semanticsLabel.contains('美团'));

  // ---- ADR-45: the in-app browser route -------------------------------------------------------
  //
  // The user asked to SEE takeout options with images. The App cannot invent a dish list, so the
  // honest route is the platform's own result page in an in-app browser -- which means the card
  // must carry the WEB url through the presenter, and must only offer the button when there is a
  // page to show.
  check('the presenter carries the web url through to the handoff view',
      enabledHandoff.url.isNotEmpty,
      'dropping it here would leave the browser button with nothing to load');
  check('browse is offered for an enabled card that has a url',
      const AgentHandoffView(
        ...
        enabled: true,
        verified: false,
        url: 'https://i.meituan.com/s/x',
      ).canBrowse);
  check('browse is NOT offered for a disabled platform',
      !const AgentHandoffView(
        platformId: 'eleme',
        label: '饿了么',
        keyword: '',
        enabled: false,
        verified: false,
        url: 'https://www.ele.me/search?keyword=',
      ).canBrowse,
      'a disabled platform must not gain a second route out of the app');
  check('browse is NOT offered when there is no url',
      !const AgentHandoffView(
        platformId: 'meituan',
        label: '美团',
        keyword: 'x',
...
void _bannedWordingChecks() {
  ...
  // ADR-44 / SPEC-U-07 section 4.3: EVERY agent string, including the flavour split of the privacy
  // claim and the consent gate's five disclosures (SPEC-C-06 section 2.2). Harvesting them here is
  // what puts them under the FF-25 red-line scan below -- a string nobody scans can say anything.
  harvest.addAll(<String>[
    UiStrings.privacyNoticeBase,
    UiStrings.privacyNoticeOfflineSuffix,
    UiStrings.privacyDialogBody,
    UiStrings.privacyEntrySpoken,
    UiStrings.privacyEntrySubtitle,
    UiStrings.agentTitle,
    UiStrings.agentEmpty,
    UiStrings.agentErrorNetwork,
    UiStrings.agentErrorInvalidKey,
    UiStrings.agentErrorService,
    UiStrings.agentErrorTruncated,
    UiStrings.agentErrorStreamBroken,
    UiStrings.agentErrorToolRounds,
    UiStrings.agentErrorToolUnparsable,
    UiStrings.agentKeyMissing,
    UiStrings.agentGoSettings,
    UiStrings.agentHandoffTitle('美团', '牛肉面'),
    UiStrings.agentHandoffButton('美团'),
    UiStrings.agentHandoffUnverified,
    UiStrings.agentHandoffClosed,
    UiStrings.agentHandoffUnopenable,
    UiStrings.agentContextTrimmed,
    UiStrings.agentCardHealthTitle,
    UiStrings.agentCardRecentTitle,
    UiStrings.agentCardAdviceTitle,
    UiStrings.agentCardFailedTitle,
    UiStrings.agentCardRangeLine('本周', 3, 1),
    UiStrings.agentCardScoreLine('88', '良好'),
    UiStrings.agentCardDimensionLine('规律性 26、结构 30'),
    UiStrings.agentCardRecordLine('面条', '12:20', '置信度 88%'),
    UiStrings.agentConsentTitle,
    UiStrings.agentConsentLead,
    UiStrings.agentConsentSendTitle,
    UiStrings.agentConsentSendBody,
    UiStrings.agentConsentNotSendTitle,
    UiStrings.agentConsentNotSendBody,
    UiStrings.agentConsentLossTitle,
    UiStrings.agentConsentLossBody,
    UiStrings.agentConsentCostTitle,
    UiStrings.agentConsentCostBody,
    UiStrings.agentConsentRevokeTitle,
    UiStrings.agentConsentRevokeBody,
    UiStrings.agentConsentCheckbox,
    UiStrings.agentConsentConfirm,
    UiStrings.agentConsentDecline,
    UiStrings.agentRevokeAction,
    UiStrings.agentInputHint,
    UiStrings.agentSend,
    UiStrings.agentRetry,
    UiStrings.apiKeyEntry,
    UiStrings.apiKeyEntrySpoken,
    UiStrings.apiKeyEntrySubtitle,
    UiStrings.apiKeyTitle,
    UiStrings.apiKeyExplanation,
    UiStrings.apiKeyFieldLabel,
    UiStrings.apiKeySave,
    UiStrings.apiKeyClear,
    UiStrings.apiKeySaved,
    UiStrings.apiKeyCleared,
    UiStrings.apiKeyNone,
    UiStrings.apiKeyInvalidShape,
    AgentPresenter.maskedKey('sk-abcdefghijklmnop'),
  ]);
