...
> # ⛔ 已被 `ADR-34` 取代 —— 本文描述的东西**已不在产品里**
>
> **2026-09-14：用户裁决「我不要 qwen 这种解释模型参与了，把软件回退到最初 … 把 qwen 端侧相关
> 代码删干净」。** 端侧语言模型层被**整体删除**：532 MB 权重、`libacoudiet_llm.so`、C shim、
> Dart 适配器、建议润色层、首页「AI 周综述」卡片、自检第 15 项，全部不在仓库里了。
>
> **因此本文以下内容只能当历史记录读，不要照它去重新装配**：文中提到的每个文件路径、每个
> 命令、每个"已完成"状态，在今天的仓库里都对不上（`git log` 里能查到的最后一个包含它的提交
> 是 `ADR-36` 的那次首提交之前的状态；删除前的完整归档在
> `_toolchain/qwen_rollback_archive/acoudiet-qwen-removed-<时间戳>/`）。
>
> **如果将来要恢复这个功能，先读 `ADR-34` 的「诚实边界」那一行**：当时"建议模型加载失败"
> 的根因**从未查明**，已排除的假设和**没做的两步**都在那里写清楚了。从那里开始，不要从重写
> 代码开始。
