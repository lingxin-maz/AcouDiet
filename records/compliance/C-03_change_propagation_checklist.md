...
rg -n --glob '!**/build/**' --glob '!**/.dart_tool/**' --glob '!_toolchain/**' `
   ...
   --glob '!records/compliance/C-03_*.md' `
...
| 范围 | 结果 |
|---|---|
| `app`（全部 Dart/Kotlin + 生成物） | **0 命中** |
| `ai` | **0 命中**（唯一疑似命中 `mobilenetv3small` 是 `3s` 后紧跟字母的假阳性 —— 故上式加了右边界守卫 `([^A-Za-z0-9]|$)`，规范值已按此细化） |
| `records/reports`、`README.md` | 0 命中 |
| `records/compliance/C-03_change_propagation_checklist.md` | 存在旧值，但**这是变更传播单自身的"旧值/新值"对照表**，属 `SPEC-C-03` §7 判据 3 明示的授权例外（与 `docs/**/SPEC-C-03_*.md` 同类），故在上式中一并排除 |
...
| 运行期抓包为 0 | 无任何 socket / http 依赖（依赖面见 `c04_dependency_deviation.md`） | ☑ 见 `records/compliance/C-01_privacy_checklist.md` |
