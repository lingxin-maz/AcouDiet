...
已经打好在 `release/dist/`（**下表为 `ADR-24`（UI 按投放的界面示意图重构）之后重新实测的值**）：
...
> 改动清单与证据见 `records/reports/adr24_ui_rebuild.md`（UI）与 `records/reports/adr23_portions_dims_refresh.md`（口径）。
...
> 也就是"UI 好像完全没变"的那个现象。根因与修复见 `records/reports/adr24_ui_rebuild.md` §9。
> 该版同时修掉了 `ADR-25`（报告页「每日」的分数与评价恒为 `--`，原因见
> `records/reports/adr25_daily_score_window.md`）。
...
归档到 `release/` 并生成索引）。它**不**写 `dist/`，所以手动拷贝那一步仍需自己做。
...
> `records/reports/u02_second_session_event_binding.md` §5）：**90 s 静默由原生自动结束会话时，页面不会自己
