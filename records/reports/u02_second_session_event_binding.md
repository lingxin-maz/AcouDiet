...
| `flutter analyze` | 0 error；**81 issues，与 `release/analyze_20260912.txt` 逐条相同**（只有我改过的 `ui_presenter_tests.dart` 里三条 info 的行号随新增行平移），即**未引入任何新问题** |
...
已按 `records/demo/PHONE_INSTALL.md §3` 的命名覆盖 `dist/AcouDiet-1.0.0-arm64-release.apk` 与
