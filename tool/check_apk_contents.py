...
actually get packaged", as opposed to "the asset exists in the source tree".
    ...
    python tool/check_apk_contents.py <apk> --expect-no-internet   # offline flavour
    python tool/check_apk_contents.py <apk> --expect-internet      # agent flavour

ADR-44 delivers two flavours, so the INTERNET assertion has two legitimate directions:
`--expect-no-internet` still means "the offline release must not request it" (the original
v1.0 evidence chain) and `--expect-internet` means "the agent release must request it".
Passing neither only *reports* the observation; passing both is refused, because a flag that
cannot be satisfied either way is not a gate.
"""
...
def main(argv: list[str]) -> int:
    ...
    expect_no_internet = "--expect-no-internet" in flags
    expect_internet = "--expect-internet" in flags
    # Refused rather than silently resolved: the two flags describe opposite flavours of the
    # same artifact, so honouring either one would make the run's verdict depend on argument
    # order. Exit 1 immediately, before any file is even opened.
    if expect_no_internet and expect_internet:
        print("ACD-ART-001: --expect-no-internet and --expect-internet are mutually "
              "exclusive; the offline flavour must not request INTERNET and the agent "
              "flavour must, so they cannot both apply to one APK.", file=sys.stderr)
        return 1
    ...
    print()
    # ADR-44: the section header names the expectation that was actually requested, so a reader
    # of the log can tell an asserted check from a bare observation.
    if expect_internet:
        print("--- INTERNET permission (must be PRESENT, agent flavour) ---")
    elif expect_no_internet:
        print("--- INTERNET permission (must be ABSENT, offline flavour) ---")
    else:
        print("--- INTERNET permission (observed, not asserted; pass --expect-no-internet or "
              "--expect-internet) ---")
    ...
    present = found_ascii or found_utf16
    print(f"  '{needle}' present in the binary manifest: "
          f"{present}  (ascii={found_ascii}, utf16le={found_utf16})")
    if expect_no_internet and present:
        print("  !! --expect-no-internet was requested and INTERNET is present -- blocker")
        return 1
    if expect_internet and not present:
        print("  !! --expect-internet was requested and INTERNET is absent -- blocker")
        return 1
    if expect_internet:
        print("  -> the agent flavour export is present, as required")
    elif expect_no_internet:
        print("  -> the offline flavour export is absent, as required")
    return 0
